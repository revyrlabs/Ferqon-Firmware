...
class FerqonEmulator:
    ...
    def _handle_frame(self, frame: bytes) -> bytes:
        ...
        # Enforce PKT_REQUEST requirement — matches firmware dispatcher.
        # DEVICE_INFO and DRIVER_INFO are exempt (may have empty payload).
        # All other commands require payload[0] == PKT_REQUEST.
        exempt = cmd_id in (CMD_DEVICE_INFO, CMD_DRIVER_INFO)

        if exempt:
            # Info commands: no PKT_REQUEST byte expected; use payload as-is.
            # Reject if extra bytes are present (firmware does the same).
            if len(payload) > 0:
                error_payload = bytes([PKT_ERROR, 2, 1, 0, 0])  # INVALID_PARAMS
                return build_frame(seq, cmd_id, error_payload)
            args = payload
        else:
            # All other commands: require PKT_REQUEST as first byte.
            if len(payload) < 1 or payload[0] != PKT_REQUEST:
                error_payload = bytes([PKT_ERROR, 2, 1, 0, 0])  # INVALID_PARAMS
                return build_frame(seq, cmd_id, error_payload)
            args = payload[1:]
