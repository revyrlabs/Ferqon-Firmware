...
Generates:
    ...
    <output_dir>/device_descriptor.h - Shared device descriptor type definitions
...
def generate_device_descriptor_h() -> str:
    """Generate the shared device descriptor header defining channel types."""
    lines = [
        f"/* {SPDX_LICENSE} */",
        f"/* {COPYRIGHT} */",
        "/**",
...
def generate_for_board(
    ...
    if check_mode:
        ...
        # Check device_descriptor.h
        generated_descriptor_h = generate_device_descriptor_h()
        ...
        device_descriptor_h = generate_device_descriptor_h()
        (output_dir / "device_descriptor.h").write_text(device_descriptor_h)
        ...
        print(f"  {output_dir / 'device_descriptor.h'}")
...
def main() -> None:
    ...
    parser.add_argument(
        "--include-dev",
        action="store_true",
        help="With --all, also process platforms/in_development/<board>/board.yml",
    )
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    if args.all:
        # Generate for all production platforms (platforms/<board>/).
        # In-development boards are only included with --include-dev because
        # their generated/ directories are gitignored and not CI-verified.
        platforms_dir = Path(__file__).parent.parent / "platforms"
        if not platforms_dir.exists():
            sys.exit(f"ERROR: platforms directory not found: {platforms_dir}")

        board_ymls = sorted(platforms_dir.glob("*/board.yml"))
        # Exclude the in_development directory itself from the top-level glob.
        board_ymls = [p for p in board_ymls if p.parent.name != "in_development"]

        if args.include_dev:
            in_dev_dir = platforms_dir / "in_development"
            if in_dev_dir.exists():
                board_ymls += sorted(in_dev_dir.glob("*/board.yml"))

        any_changes = False
        for board_yml in board_ymls:
