...
cmd_identify_dev.py
-------------------
Development-only identify command that supports the in-process emulator.

This module is NOT part of the production CLI. It imports ferqon_emulator
and ferqon_selftest which are development-only dependencies.
"""

import argparse

from ferqonfw.protocol import encode_frame as _encode_frame
...
def cmd_identify_dev(args: argparse.Namespace) -> int:
    """Run device identification (development: supports emulator)."""
    if args.emulator:
    ...
    try:
        # Only connect serial transports — emulator transports are
        # ready immediately and do not require connect().
